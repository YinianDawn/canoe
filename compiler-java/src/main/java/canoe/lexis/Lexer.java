...
public class Lexer {
    ...
    public static List<Token> parseTokens(SourceFile sourceFile, Options options) {
        return new Lexer(sourceFile, options).parse();
    }

    private SourceFile sourceFile;
    private Options options;

    private Lexer(SourceFile sourceFile, Options options) {
    ...
    private List<Token> parse() {
        ...
        for (int i = 0; i < size - 1; i++) {
            ...
            switch (token.getKind()) {
                case COLON:
                    switch (next.getKind()) {
                        case PUBLIC: if (merge(Kind.COLON_PUBLIC, token, next, "")) { i++;} continue;
                        case PROTECTED: if (merge(Kind.COLON_PROTECTED, token, next, "")) { i++;} continue;
                        case PACKAGE: if (merge(Kind.COLON_PACKAGE, token, next, "")) { i++;} continue;
                        case PRIVATE: if (merge(Kind.COLON_PRIVATE, token, next, "")) { i++;} continue;
                    ...
                    break;
                case ELSE:
                    if (next.getKind() == Kind.IF) {
                        if (merge(Kind.ELSE_IF, token, next, " ")) { i++;} continue;
                    }
                    break;
                case BIT_AND:
                    if (next.getKind() == Kind.BIT_AND) {
                        if (merge(Kind.LOGICAL_AND, token, next, "")) { i++;} continue;
                    }
                    break;
                case BIT_OR:
                    if (next.getKind() == Kind.BIT_OR) {
                        if (merge(Kind.LOGICAL_OR, token, next, "")) { i++;} continue;
                    }
                    break;
                case BIT_NOT:
                    if (next.getKind() == Kind.EQ) {
                        if (merge(Kind.NE, token, next, "")) { i++;} continue;
                    }
                    break;
                case GT:
                    if (next.getKind() == Kind.EQ) {
                        if (merge(Kind.GE, token, next, "")) { i++;} continue;
                    }
                    if (next.getKind() == Kind.GT) {
                        if (merge(Kind.BIT_MOVE_RIGHT, token, next, "")) { i++;} continue;
                    }
                    break;
                case LT:
                    if (next.getKind() == Kind.EQ) {
                        if (merge(Kind.LE, token, next, "")) { i++;} continue;
                    }
                    if (next.getKind() == Kind.LT) {
                        if (merge(Kind.BIT_MOVE_LEFT, token, next, "")) { i++;} continue;
                    }
                    break;
                case EQ:
                    if (next.getKind() == Kind.GT) {
                        if (merge(Kind.ASSIGN_IMMUTABLE, token, next, "")) { i++;} continue;
                    }
                    break;
                case SUB:
                    if (next.getKind() == Kind.GT) {
                        if (merge(Kind.ASSIGN_MUTABLE, token, next, "")) { i++;} continue;
                    }
                    break;
                case CR:
    ...
    private boolean merge(Kind kind, Token t1, Token t2, String join) {
        if (kind.getKey().equals(t1.getKind().getKey() + join + t2.getKind().getKey()) && t1.getLineNumber() == t2.getLineNumber()) {
            int i1 = t1.getIndex();
            int i2 = t2.getIndex();
            int range = i2 - i1 - t1.getLength();
            if (range == join.length()) {
                tokens.add(new Token(kind, null, t1.getFileName(), t1.getLineNumber(), t1.getIndex(), kind.name().length()));
                return true;
            }
        }
        tokens.add(t1);
    ...
    private void check(char c) {
        switch (c) {
            ...
            case '=': addToken(); addToken(Kind.EQ); index++; break;
            ...
            case '\"': addToken();
                ...
                while (reader.hasNext()) {
                    ...
                    if (next == '\r' || next == '\n') { panic("string must be single line."); }
    ...
    private void addToken(Kind kind, String value) {
        tokens.add(new Token(kind, value, sourceFile.getFileName(), lineNumber, index, null == value ? 1 : value.length()));
        if (0 < chars.length()) {
    ...
    private void addToken() {
        ...
        if (match(value, Kind.REAL_DECIMAL, "[0-9_]*\\.[0-9_]+((e|E)(-|\\+)?[1-9][0-9]*)?")) { return; }
